...
class HeatmapClassificationDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if im_type == "p":
            ...
            ret_dict = {
                "input": transforms.ToTensor()(warped_im),
                "class_pos": torch.tensor(self.center_cache[label]).float() * 256
            }
        elif im_type == "c":
            single_im = Image.open(f"data/generated/{name}")
            ...
            ret_dict = {
                "input": transforms.ToTensor()(single_im),
                "class_pos": torch.tensor(info["position"]).float()
            }
            self.cache[idx] = ret_dict
        else:
            raise ValueError(f"Unknown image type {im_type}")
        cool = torch.zeros([256, 256])
        cool[int(ret_dict["class_pos"][0]), int(ret_dict["class_pos"][1])] = 1
        ret_dict["class_pos"] = cool.flatten()
        ret_dict["input"] = torch.cat([ret_dict["input"], self.uv], dim=0)
        return ret_dict
